# TraSt-CSV
This C# console application demonstrates trajectory streaming (TraSt) for motion control systems. It enables users to stream predefined motion paths - loaded from a CSV file - to one or more motion axes. The application continuously sends trajectory segments to the drives and provides feedback. At any time, the user can safely abort the streaming process by pressing 'q'. 
